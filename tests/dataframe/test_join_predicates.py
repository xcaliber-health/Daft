...
def _rows(frame):
    out = frame.sort(["k", "lv", "rv"], nulls_first=True).to_pydict()
    return list(zip(out["k"], out["lv"], out["k2"], out["rv"]))
...
def test_inner_join_keeps_only_pairs_the_condition_allows(sides):
    ...
    assert rows == [(1, 10, 1, 12), (1, 11, 1, 12), (2, 20, 2, 25)]
...
def test_left_join_reports_a_row_whose_every_pair_fails_as_unmatched(sides):
    ...
    assert rows == [(1, 10, 1, 12), (1, 11, 1, 12), (2, 20, 2, 25), (3, 30, None, None)]
...
def test_a_condition_no_pair_satisfies_leaves_every_row_unmatched(sides):
    ...
    assert rows == [(1, 10, None, None), (1, 11, None, None), (2, 20, None, None), (3, 30, None, None)]
...
def test_key_equality_alone_is_unaffected(sides):
    ...
    assert rows == [(1, 10, 1, 5), (1, 10, 1, 12), (1, 11, 1, 5), (1, 11, 1, 12), (2, 20, 2, 25)]
